package com.spring.learning.jobportal.user.service.impl;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.spring.learning.jobportal.constants.ApplicationConstants;
import com.spring.learning.jobportal.dto.*;
import com.spring.learning.jobportal.entity.*;
import com.spring.learning.jobportal.repository.*;
import com.spring.learning.jobportal.user.service.IUserService;
...
public class UserServiceImpl implements IUserService {
    ...
    public List<JobDto> getSavedJobs(String userEmail) {
        ...
        JobPortalUser user = userRepository.findJobPortalUserByEmail(userEmail)
                .orElseThrow(() -> new RuntimeException("User not found with email: " + userEmail));
        return user.getSavedJobs().stream().map(job -> ApplicationUtil.transformJobToDto(job))
    ...
    public JobApplicationDto applyForJob(String userEmail, ApplyJobRequestDto applyJobRequestDto) {
        ...
        return ApplicationUtil.mapToJobApplicationDto(saved);
    }
    ...
    public List<JobApplicationDto> getJobSeekerApplications(String userEmail) {
        ...
        JobPortalUser user = userRepository.findJobPortalUserByEmail(userEmail)
                .orElseThrow(() -> new RuntimeException("User not found with email: " + userEmail));
        return user.getJobApplications().stream().map(ApplicationUtil::mapToJobApplicationDto)
                .collect(Collectors.toList());
    }
