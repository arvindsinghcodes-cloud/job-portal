package com.spring.learning.jobportal.contact.controller;
...
public class ContactController {
    ...
    public ResponseEntity<String> fetchOpenContacts(@RequestParam
                                                       @Validated @NotBlank(message = "status cannot be blank")
                                                        @Size(min=5,message = "Status length min should be 5 chars") String status){
        return ResponseEntity.ok("These are the open contacts with given "+status);
    }
