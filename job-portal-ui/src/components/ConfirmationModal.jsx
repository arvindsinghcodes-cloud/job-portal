...
const ConfirmationModal = ({
  ...
  // Optional input (e.g. cover letter)
  showInput = false,
  inputValue = '',
  onInputChange = () => {},
  inputLabel = '',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-md w-full p-8 transform transition-all animate-scale-in">
        ...
        {showInput && (
          <div className="mb-6">
            {inputLabel && <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{inputLabel}</label>}
            <textarea
              value={inputValue}
              onChange={(e) => onInputChange(e)}
              placeholder={inputPlaceholder}
              className="w-full min-h-[100px] p-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 resize-none"
            />
          </div>
        )}
