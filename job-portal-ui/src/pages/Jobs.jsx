...
const Jobs = () => {
  ...
  const formatSalary = (min, max) => {
    const toDisplay = (v) => {
      ...
      // If value is large (>=1000) show in 'k'
      if (Math.abs(n) >= 1000) {
    ...
    const a = toDisplay(min);
    const b = toDisplay(max);
    if (a && b) return `${a} - ${b}`;
    if (a) return a;
    if (b) return b;
    return 'Negotiable';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-800 pt-20">
      ...
      <ConfirmationModal
        ...
        showInput={true}
        inputValue={coverLetter}
        onInputChange={(e) => setCoverLetter(e.target.value)}
        inputLabel="Cover Letter (optional)"
        inputPlaceholder="Write a short cover letter (optional)"
