...
const JobApplicants = () => {
  ...
  const handleStatusSelect = (applicationId, newStatus) => {
  ...
  const handleStatusChange = async (applicationId, newStatus) => {
    const notes = pendingNotes[applicationId];
    try {
      ...
      // Clear pending status change and notes
      setPendingStatusChanges(prev => {
        const updated = { ...prev };
  ...
  const filteredApplications = applications.filter(app => {
    ...
    const statusUpperCase = app.applicant.status?.toUpperCase() || '';
    const filterMap = {
      pending: 'PENDING',
      inreview: 'IN_REVIEW',
      interview: 'INTERVIEW',
    ...
    return statusUpperCase === filterMap[filter];
  });

  const statusCounts = {
    all: applications.length,
    pending: applications.filter(app => app.applicant.status?.toUpperCase() === 'PENDING').length,
    inreview: applications.filter(app => app.applicant.status?.toUpperCase() === 'IN_REVIEW').length,
    interview: applications.filter(app => app.applicant.status?.toUpperCase() === 'INTERVIEW').length,
    hired: applications.filter(app => app.applicant.status?.toUpperCase() === 'HIRED').length,
    rejected: applications.filter(app => app.applicant.status?.toUpperCase() === 'REJECTED').length,
  };
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="lg:ml-8 lg:text-right">
                    <div className="mb-4">
                      ...
                      <select
                        value={pendingStatusChanges[application.applicationId] || application.applicant.status}
                        onChange={(e) => handleStatusSelect(application.applicationId, e.target.value)}
                        className="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-300 outline-none mb-3"
                      >
                      ...
                      <textarea
                        value={pendingNotes[application.applicationId] ?? (application.applicant.notes || '')}
                        onChange={(e) => handleNotesChange(application.applicationId, e.target.value)}
                        placeholder="Add internal notes about this applicant..."
                        rows={3}
                        className="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-300 outline-none resize-none text-sm mb-3"
                      />

                      <button
                        onClick={() => handleStatusChange(application.applicationId, pendingStatusChanges[application.applicationId] || application.applicant.status)}
                        className="w-full px-4 py-2 bg-green-600 dark:bg-green-700 text-white rounded-xl hover:bg-green-700 dark:hover:bg-green-600 transition-colors font-semibold text-sm"
                      >
                        Update
                      </button>
                    </div>
