...
export const JobProvider = ({ children }) => {
      ...
      if (user.role === 'ROLE_JOB_SEEKER') {
        ...
        const loadAppliedJobs = async () => {
          try {
            ...
            // Transform backend data to match frontend format and normalize salary fields
            const transformedApplications = applicationsData.map(app => {
              const job = app.job || {};
              const salaryMin = job.salary?.min ?? job.salaryMin ?? null;
              const salaryMax = job.salary?.max ?? job.salaryMax ?? null;
  ...
  const applyForJob = async (job, coverLetter = '') => {
    ...
    try {
      ...
      // Update local state and normalize salary fields
      const salaryMin = job.salary?.min ?? job.salaryMin ?? null;
      const salaryMax = job.salary?.max ?? job.salaryMax ?? null;
      ...
      // Prepend so newest appears first immediately
